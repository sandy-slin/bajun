...
export const sectorApi = {
  ...
  getTopSectors: (lookbackMonths: number = 6, topN: number = 5) =>
    api.get<ApiResponse>(`/api/v1/sectors/`, {
      params: { lookback_months: lookbackMonths, top_n: topN }
    }),
  
  // 获取板块列表
  getSectorList: () =>
    api.get<ApiResponse>('/api/v1/sectors/list'),
  
  // 获取单个板块分析
  getSectorAnalysis: (sectorName: string, analysisDays: number = 30) =>
    api.get<ApiResponse>(`/api/v1/sectors/${sectorName}`, {
      params: { analysis_days: analysisDays }
    }),
  
  // 板块性能验证
  validatePerformance: () =>
    api.get<ApiResponse>('/api/v1/sectors/performance/validation'),
};
...
export const stockApi = {
  ...
  selectStocks: (sectors: string[], stocksPerSector: number = 5) =>
    api.post<ApiResponse>('/api/v1/stocks/select', {
      sectors,
...
export const portfolioApi = {
  ...
  analyzePortfolio: (holdings: any[]) =>
    api.post<ApiResponse>('/api/v1/portfolio/analyze', { holdings }),
  
  // 获取预设组合
  getPresetPortfolio: () =>
    api.get<ApiResponse>('/api/v1/portfolio/preset'),
  
  // 组合优化
  optimizePortfolio: (holdings: any[], optimizationTarget: string = 'risk_return') =>
    api.post<ApiResponse>('/api/v1/portfolio/optimize', holdings, {
      params: { optimization_target: optimizationTarget }
    }),
  
  // 风险评估
  assessRisk: (portfolioValue: number, riskTolerance: string = 'medium') =>
    api.get<ApiResponse>('/api/v1/portfolio/risk-assessment', {
  ...
  backtest: (startDate: string, endDate: string, rebalanceFrequency: string = 'monthly') =>
    api.get<ApiResponse>('/api/v1/portfolio/performance/backtest', {
      params: { 
...
export const tradingApi = {
  ...
  checkTradingDecision: (request: any) =>
    api.post<ApiResponse>('/api/v1/trading/check', request),
  
  // 情绪控制建议
  getEmotionControl: (request: any) =>
    api.post<ApiResponse>('/api/v1/trading/emotion-control', request),
  
  // 获取交易规则
  getTradingRules: () =>
    api.get<ApiResponse>('/api/v1/trading/rules'),
  
  // 更新交易规则
  updateTradingRules: (rules: any) =>
    api.put<ApiResponse>('/api/v1/trading/rules', null, { params: rules }),
  
  // 获取交易历史
  getTradingHistory: (days: number = 30, analysisType: string = 'summary') =>
    api.get<ApiResponse>('/api/v1/trading/history', {
...
export const systemApi = {
  ...
  // 系统信息
  getSystemInfo: () =>
    api.get<ApiResponse>('/api/v1/system/info'),
  
  // 系统状态
  getSystemStatus: () =>
    api.get<ApiResponse>('/api/v1/system/status'),
};
